...
const navItems = [
  ...
  { to: '/admin/lojas', label: 'Lojas', icon: LayoutGrid },
  { to: '/admin/funcionarios', label: 'Funcionários', icon: Users },
  { to: '/admin/produtos', label: 'Produtos', icon: Package },
  { to: '/admin/chamativos', label: 'Chamativos', icon: Megaphone },
  { to: '/admin/desafios', label: 'Desafios', icon: Trophy },
];
    ...
    <div className="min-h-screen bg-surface-muted flex flex-col lg:flex-row">
      ...
      >
        <div className="p-5 sm:p-6 border-b border-white/10 flex items-start justify-between gap-3">
          <div>
            <Link to="/admin" className="flex items-center gap-2.5 font-bold text-lg">
              <Store className="w-6 h-6" />
              Admin
            </Link>
            <p className="text-white/60 text-xs mt-1">Revista MercadoFácil</p>
          </div>
          <button
            type="button"
      ...
      <div className="flex-1 flex flex-col min-w-0">
        <header className="lg:hidden sticky top-0 z-30 bg-white border-b border-border/60 px-4 py-3 flex items-center gap-3">
          ...
          <span className="font-semibold text-navy truncate">Painel Admin</span>
        </header>
